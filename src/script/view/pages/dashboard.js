    ...
    async afterRender() {
        ...
        async function loadRecentActivities() {
            ...
            items.forEach(item => {
                ...
                li.className = 'text-sm text-gray-800 border-b py-2 flex justify-between items-center';
                ...
                li.innerHTML = `
                    <span>${item.prediction} - ${formatted}</span>
            ...
            document.querySelectorAll('.delete-btn').forEach(button => {
                button.addEventListener('click', async (e) => {
                    ...
                    if (confirmDelete.isConfirmed) {
                        const success = await presenter.deletePredictions(id);
                        if (success) {
                            await Swal.fire({
                                icon: 'success',
